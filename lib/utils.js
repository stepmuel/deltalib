...
function deepEmpty(obj) {
  for (var key in obj) {
    if (!obj.hasOwnProperty(key)) continue;
    var o = obj[key];
    if (!isMergeable(o)) return false;
    if (!deepEmpty(o)) return false;
  }
  return true;
}
module.exports.deepEmpty = deepEmpty;
...
// FIXME: null values should be removed from clones, if del == true

function _merge(d, p, del) {
  for (var k in p) {
    var v = p[k];
    if (del && v === null) {
      delete d[k];
